...
export const signIn = async (provider = 'cognito', options = {callbackUrl: '/'}) => {
    if (provider === 'cognito') {
        ...
        const authState = cryptr.encrypt(`${canonicalUri}/?secret=${authSecret}&follow=${encodeURIComponent(followUri)}`)

        const auth_endpoint = `https://auth.almunia.io/authorize?response_type=code&state=${authState}&client_id=${clientId}&redirect_uri=${redirectUri}`
